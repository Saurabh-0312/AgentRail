 ...
 * GET /api/status
 *
 * A real health dot per chain: one cheap read each, timed, from the server. Plus the index heads
 * in Studio and whether the live x402 feed is awake (Render's free tier sleeps). Nothing here is a
...
export async function GET() {
  const [sep, bas, hed, sol, feed, idxSep, idxBase] = await Promise.all([
    ...
    timed("feed", "x402 feed (Render)", async () => {
  ...
  const chains = [sep, bas, hed, sol];
  return Response.json({ at: new Date().toISOString(), chains, feed: { ...feed, sleeping: !feed.ok && /timeout/.test(feed.detail) }, index: [idxSep, idxBase] }, { headers: { "Cache-Control": "no-store" } });
}
