...
/**
 * Tables for anything with more than three rows. Wide content scrolls inside its own box; from
 * the `xl` breakpoint, where the six-column layout is known to fit, the box is open so a sticky
 * header can pin to the page.
 */
export function Table({ className, dense = false, stickyHeader = false, ...props }: HTMLAttributes<HTMLTableElement> & { dense?: boolean; stickyHeader?: boolean }) {
  return (
    <div className={cn("max-w-full overflow-x-auto rounded-xl border border-border bg-surface shadow-1", stickyHeader && "xl:overflow-visible")}>
...
export function THead({ className, sticky = false, ...props }: HTMLAttributes<HTMLTableSectionElement> & { sticky?: boolean }) {
  return <thead className={cn("bg-surface-sunken text-xs uppercase tracking-wide text-muted", sticky && "xl:sticky xl:top-14 xl:z-10 xl:shadow-1", className)} {...props} />;
}
